...
function Paddle:Track(ball, PADDLE_SPEED,dt)
    if ball.y > self.y + 8 then
        self.dy = PADDLE_SPEED
    elseif ball.y < self.y +8 then
        self.dy = -PADDLE_SPEED
    end

    if self.y + 8 ~= ball.y then

        if self.dy < 0 then
            self.y = math.max(ball.y - 8,self.y + self.dy * dt)
        else
            self.y = math.min(ball.y - 8,self.y + self.dy * dt)
        end
    end
end
